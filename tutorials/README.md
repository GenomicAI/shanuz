...
| 18 | [Anchor Internals — CCA & RPCA](anchors_vignette.md) | 2,400 cells · CTRL/STIM · ifnb (Kang 2018) | The v4 anchor path at the level of the anchors themselves, not the clustering they produce: `FindIntegrationAnchors` ↔ `find_integration_anchors` · `IntegrateData` ↔ `integrate_data` · `RunCCA` · `ReciprocalProject` · `ScoreAnchors` · `FilterAnchors` · `FindWeights` — **RPCA agrees on 100 % of Seurat's anchors** (649/649, 99.5 % of scores identical) and CCA on **99.9 %**, up from 70.0 %; corrected expression to `mean|Δ| 0.0993` vs Seurat's `0.0992`. **Caught twelve bugs, all fixed** — CCA standardizing vs L2-normalizing, a score rescaled against anchors that were then discarded, a Gaussian where Seurat uses `1 − exp(−d̃·score/(2/sd)²)`, and sklearn's randomized SVD drifting the trailing PCs enough to move every reciprocal-PCA anchor | Advanced |
