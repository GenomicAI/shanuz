...
model around: :func:`truecell.run_umap` stashes it in
...
and running a short, held-fixed optimisation. So projection is a two-step map:
  ...
  1. **query → reference PCA.** The reference's PCA is a plain linear map from
     ...
     :func:`truecell.find_transfer_anchors`'s ``pcaproject`` robust.
...
workflow: take the anchors, :func:`~truecell.transfer_data` the labels onto the
