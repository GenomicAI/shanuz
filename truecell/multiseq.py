...
problem as :func:`truecell.hto_demux` solves — turn the per-cell barcode count
...
1. **Normalize.** Barcode counts are compositional, so they are centered-log-ratio
   (CLR) normalized — the same transform :func:`truecell.normalize_data` applies with
   ``method="CLR"`` (margin 1, per barcode across cells, as Seurat's default does). As with
   :func:`truecell.hto_demux`, ``normalize=True`` (default) recomputes this from the
