...
| Path | What |
|---|---|
| `shanuz/` | The package. 53 source modules; `spatial/`, `compat/`, `mixins/` are subpackages. |
...
Scope is pinned in `pyproject.toml` (`[tool.mypy] files = ["shanuz"]`), so a bare
`mypy` checks exactly what CI checks. `ruff` takes scope from the command line —
`ruff check shanuz` and `ruff check .` are both correct and easy to confuse when
comparing counts.

`E402` is ignored for `tests/*` and `tutorials/*` by scope: each file bootstraps
`sys.path` before importing shanuz so it runs straight from a clone, which puts
...
installs the wheel **clean into `/tmp`** and asserts `shanuz.__version__` matches
`pyproject.toml`. That catches what the editable test job cannot: whether the
wheel is complete, whether a plain `pip install shanuz` works without the
...
`~/.shanuz_data/` (~770 MB for the full set).
...
SHANUZ_TUTORIAL_SMOKE=1 pytest tests/test_tutorial_smoke.py -v   # real runs, needs cached data
```
...
```bash
mkdocs serve            # watches docs/, shanuz/ and tools/
mkdocs build --strict
```
...
- mkdocstrings reads annotations **statically** through griffe, so the 19
  `if TYPE_CHECKING` annotations (matplotlib's `Figure`, `Neighbor`, `Shanuz`)
