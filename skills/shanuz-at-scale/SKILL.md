---
name: shanuz-at-scale
description: Use when a dataset is too large to analyse in RAM with shanuz — leverage-score sketching (leverage_score, sketch_data, project_data) to analyse a representative subset and extend the result to every cell, and LazyMatrix (write_lazy_matrix, open_lazy_matrix, col_blocks) to keep the counts matrix memory-mapped on disk.
---

# Working at scale

Load the `shanuz` skill first. Two independent tools; they compose.
...
sketch = shanuz.sketch_data(obj, ncells=50_000, method="LeverageScore")
# sketch is a NEW standalone Shanuz object — rebind it

# Run the expensive analysis on the sketch only
shanuz.scale_data(sketch)
shanuz.run_pca(sketch, n_pcs=50)
shanuz.find_neighbors(sketch, dims=range(30))
shanuz.find_clusters(sketch, resolution=0.5)
shanuz.run_umap(sketch, dims=range(30))
...
lose them. Leverage tracks rarity at **−0.929** in both shanuz and Seurat.

`method="Uniform"` is the control, not a lesser option: run it to see what
leverage weighting is buying you on *your* data.

```python
scores = shanuz.leverage_score(obj, features=hvg, nsketch=5000, seed=123)
```
...
Seurat stores the sketch as an extra assay on the same object. **shanuz returns a
...
```python
from shanuz import write_lazy_matrix, open_lazy_matrix, is_lazy
...
shanuz matrices are features × cells, and the operations that dominate at scale
...
shanuz's on-disk and in-memory paths are **bit-identical**. (Seurat's differ by
...
sketch = shanuz.sketch_data(obj, ncells=50_000)
# ... full analysis on `sketch` ...
shanuz.project_data(obj, sketch, refdata={"celltype": "seurat_clusters"})
```

## Reference

- [Leverage-score sketching](https://genomicai.github.io/shanuz/tutorials/sketch_vignette/) — both of Seurat's regimes, with the uniform control.
- [Out of core: `LazyMatrix` vs BPCells](https://genomicai.github.io/shanuz/tutorials/lazy_vignette/) — 14 of 14 anchors, and the memory measurements.
