...
Signatures are from `shanuz` 0.9.0. `seurat` / `obj` as the first parameter means
a `Shanuz` object.
...
log_shanuz_command(object_, func_name, params=None, assay=None, reduction=None) -> ShanuzCommand
as_graph(x, cell_names=None, assay_used=None, weighted=True) -> Graph
```

Classes exported: `Shanuz`, `Assay`, `Assay5`, `StdAssay`, `DimReduc`, `Graph`,
`Neighbor`, `JackStrawData`, `LogMap`, `KeyMixin`, `ShanuzCommand`.
...
from shanuz.datasets import pbmc3k, pbmc8k, cbmc_citeseq, pbmc_hashing, thp1_eccite, \
...
Everything caches to `~/.shanuz_data/` (~770 MB for the full set).

```python
from shanuz.compat.anndata import as_anndata, from_anndata
as_anndata(seurat, assay=None)
from_anndata(adata, assay="RNA", spatial_key="spatial", fov_key="fov") -> Shanuz
```

## Preprocessing

| Seurat | shanuz | Returns |
|---|---|---|
...
- `scale_data(features=None)` scales the **variable features**, as Seurat does.
  Pass `features=shanuz.generics.features(obj)` for `ScaleData(features = rownames(obj))`.
...
## Differential expression → `shanuz-differential-expression`
...
## Integration and mapping → `shanuz-integration`
...
## Demultiplexing and screens → `shanuz-multimodal`
...
## Generics (`shanuz.generics.*`, not top-level)
