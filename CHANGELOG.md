...
- **A logo** (`docs/assets/logo/`), generated by `tools/make_logo.py` rather
  than drawn — the mark is a point cloud whose density traces a lowercase `s`,
  which is the nod to Georges Seurat, the pointillist the R package is named
  for. The `s` is two lobes meeting at the waist in two colours: the two
  implementations, one shape. Ships as a mark, a horizontal lockup, a wordmark,
