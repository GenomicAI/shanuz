...
from truecell.lazy import LazyMatrix, write_lazy_matrix  # noqa: E402
from truecell.preprocessing import _log_normalize, _loess2, _vst_hvg  # noqa: E402
...
def test_qc_metadata_matches_between_a_store_and_a_sparse_matrix(tmp_path):
    ...
    built = [
        create_truecell_object(counts=m, assay="RNA", feature_names=genes,
...
def test_whole_pipeline_never_materialises_the_store(tmp_path):
    ...
    with _MaterialisationCounter() as counter:
        truecell.percentage_feature_set(obj, pattern=r"^g1", col_name="pct")
        truecell.normalize_data(obj)
        truecell.find_variable_features(obj, nfeatures=20)
        truecell.scale_data(obj, features=assay.variable_features)
        truecell.run_pca(obj, n_pcs=5, features=assay.variable_features,
                       reduction_name="pca")
        truecell.add_module_score(obj, features=[list(assay.features())[:10]],
                                name="mod")
        truecell.find_markers(obj, ident_1="a", ident_2="b")
...
def test_downstream_steps_never_materialise_a_lazy_data_layer(tmp_path):
    ...
    with _MaterialisationCounter() as counter:
        truecell.find_variable_features(obj, nfeatures=20)
        truecell.scale_data(obj, features=[f"g{i}" for i in range(30)])
        truecell.add_module_score(obj, features=[[f"g{i}" for i in range(5)]],
                                name="mod", seed=0)
        truecell.find_markers(obj, ident_1="a", ident_2="b")
...
def test_streaming_normalize_is_invariant_to_block_size(tmp_path, block_size):
    from truecell.preprocessing import _log_normalize_lazy
...
def test_percentage_feature_set_works_on_a_lazy_layer(tmp_path):
    ...
    truecell.percentage_feature_set(sparse_obj, pattern=r"^g1", col_name="pct")
    truecell.percentage_feature_set(lazy_obj, pattern=r"^g1", col_name="pct")
...
def test_find_markers_agrees_between_lazy_and_sparse(tmp_path):
    ...
    for obj in (sparse_obj, lazy_obj):
        obj.idents = ["a"] * 40 + ["b"] * 40
        out.append(truecell.find_markers(obj, ident_1="a", ident_2="b"))
...
def test_add_module_score_agrees_between_lazy_and_sparse(tmp_path):
    ...
    for obj in (sparse_obj, lazy_obj):
        truecell.add_module_score(obj, features=[program], name="mod", seed=0)
...
def test_loess_gives_one_fitted_value_per_distinct_x():
    ...
    spread on pbmc3k is 1.8e-15, pure interpolation noise. truecell's was
